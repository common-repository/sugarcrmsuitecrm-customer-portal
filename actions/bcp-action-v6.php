...
if ( ! function_exists( 'scp_sign_up_callback' ) ) {
    add_action( 'admin_post_scp_sign_up', 'scp_sign_up_callback' );    
    add_action( 'admin_post_nopriv_scp_sign_up', 'scp_sign_up_callback' );
    function scp_sign_up_callback() {
        ...
        if ( $sess_id != '' ) {
            ...
            if ( isset( $username_c ) && $username_c != '' ) {
                $addSignUp = array(
                ...
                if ( ( $checkUserExists ) && ( $checkEmailExists ) && ( $user_id ) && $wp_email_exist ) {
                    ...
                    if ( $isSignUp != NULL ) {
                        ...
                        scp_mail_user( $username_c, $password_c, $first_name, $last_name, $email1 );
...
if ( ! function_exists( 'scp_login_callback' ) ) {
    add_action( 'admin_post_scp_login', 'scp_login_callback' );
    add_action( 'admin_post_nopriv_scp_login', 'scp_login_callback' );
    function scp_login_callback() {
        ...
        if ( $sess_id != '' ) {
            ...
            if ( ( $isLogin->entry_list[0] != NULL) && ( $scp_username != NULL ) && ( $scp_password != NULL ) ) {
                ...
                $biztech_redirect_manage = get_page_link(get_option('biztech_redirect_manange'));
    ...
    function scp_forgot_password_callback() {
        ...
        if ( $sess_id != '' ) {
            ...
            if ( ( $username == $checkUsername ) && ( $emailAddress == $checkEmialAddress ) ) {
                ...
                $isSendEmail = scp_mail_user_forgotpwd( $username, $password, $url, $emailAddress );
                if ( $isSendEmail == true ) {
...
if ( ! function_exists( 'scp_list_module_callback' ) ) {
    ...
    function scp_list_module_callback() {
        ...
        if ($sess_id != '') {
            ...
            $offset = ((int) (($page_no * $limit) - $limit));
            $order_by = sanitize_text_field( trim($_POST['order_by']) );
            $order = sanitize_text_field( trim($_POST['order']) );
            $search_name = array_key_exists('searchval', $_POST) ? trim(sanitize_text_field( $_POST['searchval'])) : null;
            ...
            $list_result_count_all = $objSCP->get_relationships('Contacts', $_SESSION['scp_user_id'], strtolower($module_name), array('id'), $where_con, $order_by_query, 0);
            $list_result = $objSCP->get_relationships('Contacts', $_SESSION['scp_user_id'], strtolower($module_name), array('id', 'name', 'case_number', 'priority', 'status', 'date_entered'), $where_con, $order_by_query, 0, $offset, $limit);
            ...
            $html .= " <div class='search-box'><input type='text' name='search_name' value='" . $search_name . "' id='search_name' placeholder='Search $module_name' class='search-input'/>
                <a href='javascript:;' onclick='scp_module_paging(0,\"$module_name\",1,\"\",\"\",\"$view\",\"$current_url\")' id='search_btn_id' class='hover active scp-button search-btn'><i class='fa fa-search' aria-hidden='true'></i>SEARCH</a></div>
            ...
            if ($cnt > 0) {
                ...
                include( SCP_TEMPLATE_PATH . 'scp_list_page_v6.php');
                $html .="</table>";
                $html .="</div>";
                $pagination_url = "?";
                $view1 = 'list';
...
if ( ! function_exists( 'scp_view_module_callback' ) ) {
    ...
    function scp_view_module_callback() {
        ...
        if ($sess_id != '') {
            ...
            $current_url = sanitize_text_field( ['current_url'] );
            $html = '';
            ...
            include( SCP_TEMPLATE_PATH . 'scp_view_page_v6.php');
            $html .= "</div>";
            echo $html;
            wp_die();
        } else {
...
if ( ! function_exists( 'scp_add_module_callback' ) ) {
    ...
    function scp_add_module_callback() {
        ...
        if ($sess_id != '') {
            ...
            $html = "";
            include( SCP_TEMPLATE_PATH . 'scp_add_page_v6.php');
